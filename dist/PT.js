...
function solve(P, T, v) {
    ...
    var region = findRegion_PT(P, T), result = null;
    switch (region) {
        case 1:
            result = r1(P, T);
            break;
...
function r1(P, T) {
    ...
    for (var i = 0; i < 34; i++) {
        var I = R1_PT_I[i], J = R1_PT_J[i], N = R1_PT_N[i];
...
function r2(P, T) {
    ...
    var gr = 0, gr_p = 0, gr_pp = 0, gr_t = 0, gr_tt = 0, gr_pt = 0;
    for (var j = 0; j < 43; j++) {
        N = R2R_PT_N[j];
        J = R2R_PT_J[j];
        I = R2R_PT_I[j];
...
function r3(P, T, rho) {
    ...
    for (var i = 1; i < 40; i++) {
        var N = R3_PT_N[i], J = R3_PT_J[i], I = R3_PT_I[i];
...
function r3_PT_v(P, T) {
    var Tsat = r4_P_Tsat(P), b3cd = b3cd_P_T(P);
    if (40.0 < P && P <= 100.0) {
        if (T <= b3ab_P_T(P)) {
            return r3a_PT_v(P, T);
        }
...
function b3ab_P_T(P) {
    var B3ab_PT_I = [0, 1, 2, -1, -2], B3ab_PT_N = [1.54793642129415E3, -1.87661219490113E2, 2.13144632222113E1, -1.91887498864292E3, 9.18419702359447E2], T = 0;
    P = Math.log(P);
    for (var i = 0; i < 5; i++) {
...
// Exported for testing
function b3op_P_T(P) {
    var B3op_PT_I = [0, 1, 2, -1, -2], B3op_PT_N = [9.69461372400213E2, -3.32500170441278E2, 6.4285959846667E1, 7.73845935768222E2, -1.52313732937084E3], T = 0;
    P = Math.log(P);
    for (var i = 0; i < 5; i++) {
...
// Exported for testing
function b3cd_P_T(P) {
    var B3cd_PT_I = [0, 1, 2, 3], B3cd_PT_N = [5.85276966696349E2, 2.78233532206915, -1.27283549295878E-2, 1.59090746562729E-4], T = 0;
    for (var i = 0; i < 4; i++) {
...
// Exported for testing
function b3gh_P_T(P) {
    var B3gh_PT_I = [0, 1, 2, 3, 4], B3gh_PT_N = [-2.49284240900418E4, 4.28143584791546E3, -2.6902917314013E2, 7.51608051114157, -7.87105249910383E-2], T = 0;
    for (var i = 0; i < 5; i++) {
...
// Exported for testing
function b3ij_P_T(P) {
    var B3ij_PT_I = [0, 1, 2, 3, 4], B3ij_PT_N = [5.84814781649163E2, -6.16179320924617E-1, 2.60763050899562E-1, -5.87071076864459E-3, 5.15308185433082E-5], T = 0;
    for (var i = 0; i < 5; i++) {
...
// Export for testing
function b3jk_P_T(P) {
    var B3jk_PT_I = [0, 1, 2, 3, 4], B3jk_PT_N = [6.17229772068439E2, -7.70600270141675, 6.97072596851896E-1, -1.57391839848015E-2, 1.37897492684194E-4], T = 0;
    for (var i = 0; i < 5; i++) {
...
// Exported for testing
function b3mn_P_T(P) {
    var B3mn_PT_I = [0, 1, 2, 3], B3mn_PT_N = [5.35339483742384E2, 7.61978122720128, -1.58365725441648E-1, 1.92871054508108E-3], T = 0;
    for (var i = 0; i < 4; i++) {
...
// Exported for testing
function b3qu_P_T(P) {
    var B3qu_PT_I = [0, 1, 2, 3], B3qu_PT_N = [5.65603648239126E2, 5.29062258221222, -1.02020639611016E-1, 1.22240301070145E-3], T = 0;
    for (var i = 0; i < 4; i++) {
...
// Exported for testing
function b3rx_P_T(P) {
    var B3rx_PT_I = [0, 1, 2, 3], B3rx_PT_N = [5.84561202520006E2, -1.02961025163669, 2.43293362700452E-1, -2.94905044740799E-3], T = 0;
    for (var i = 0; i < 4; i++) {
...
function r3c_PT_v(P, T) {
    ...
    for (var i = 0; i < 35; i++) {
        var N = R3c_PT_N[i], I = R3c_PT_I[i], J = R3c_PT_J[i];
...
function r3d_PT_v(P, T) {
    ...
    for (var i = 0; i < 38; i++) {
        var N = R3d_PT_N[i], I = R3d_PT_I[i], J = R3d_PT_J[i];
...
function r3e_PT_v(P, T) {
    ...
    for (var i = 0; i < 29; i++) {
        var N = R3e_PT_N[i], I = R3e_PT_I[i], J = R3e_PT_J[i];
...
function r3s_PT_v(P, T) {
    ...
    for (var i = 0; i < 29; i++) {
        var N = R3s_PT_N[i], I = R3s_PT_I[i], J = R3s_PT_J[i];
        v += N * Math.pow(pi - 0.886, I) * Math.pow(sig - 0.990, J);
    }
    return 0.0022 * Math.pow(v, 4);
}
function r3t_PT_v(P, T) {
    var pi = P / 20, sig = T / 650, v = 0, R3t_PT_I = [0, 0, 0, 0, 1, 1, 2, 2, 2, 3, 3, 4, 4, 7, 7, 7, 7, 7, 10, 10, 10, 10, 10, 18, 20, 22, 22, 24, 28, 32, 32, 32, 36], R3t_PT_J = [0, 1, 4, 12, 0, 10, 0, 6, 14, 3, 8, 0, 10, 3, 4, 7, 20, 36, 10, 12, 14, 16, 22, 18, 32, 22, 36, 24, 28, 22, 32, 36, 36], R3t_PT_N = [1.55287249586268, 6.64235115009031, -2.8936623672721E3, -3.85923202309848E12, -2.91002915783761, -8.29088246858083E11, 1.76814899675218, -5.34686695713469E8, 1.60464608687834E17, 1.96435366560186E5, 1.56637427541729E12, -1.78154560260006, -2.29746237623692E15, 3.85659001648006E7, 1.10554446790543E9, -6.77073830687349E13, -3.27910592086523E30, -3.41552040860644E50, -5.27251339709047E20, 2.45375640937055E23, -1.68776617209269E26, 3.58958955867578E28, -6.56475280339411E35, 3.55286045512301E38, 5.6902145441327E57, -7.00584546433113E47, -7.05772623326374E64, 1.66861176200148E52, -3.00475129680486E60, -6.68481295196808E50, 4.28432338620678E68, -4.44227367758304E71, -2.81396013562745E76];
    for (var i = 0; i < 33; i++) {
        var N = R3t_PT_N[i], I = R3t_PT_I[i], J = R3t_PT_J[i];
...
// Exported for testing
function b3uv_P_T(P) {
    var B3uv_PT_I = [0, 1, 2, 3], B3uv_PT_N = [5.28199646263062E2, 8.90579602135307, -0.222814134903755, 0.286791682263697E-2], T = 0;
    for (var i = 0; i < 4; i++) {
...
// Exported for testing
function b3wx_P_T(P) {
    var B3wx_PT_I = [0, 1, 2, -1, -2], B3wx_PT_N = [7.2805260914538, 9.73505869861952E1, 1.47370491183191E1, 0.329196213998375E3, 8.73371668682417E2], T = 0;
    P = Math.log(P);
    for (var i = 0; i < 5; i++) {
